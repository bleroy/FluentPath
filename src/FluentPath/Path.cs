...
namespace Fluent.IO {
    ...
    public sealed class Path : IEnumerable<Path> {
        private IEnumerable<string> _paths;
        private Path _previousPaths;

        /// <summary>
        /// Creates an empty Path object.
        /// </summary>
        public Path() : this(new string[] { })
        {
        }
        ...
        public Path(params string[] paths) : this((IEnumerable<string>)paths)
        {
        }
        ...
        /// </summary>
        /// <param name="paths">The list of path strings.</param>
        public Path(IEnumerable<string> paths) : this(paths, null)
        {
        }
        ...
        private static Path Create(IEnumerable<string> paths, Path previousPaths)
            => new Path {_paths = paths, _previousPaths = previousPaths};

        private static Path Create(string path) => Create(path, (Path) null);

        private static Path Create(string path, Path previousPaths)
            => new Path {_paths = new[] {path}, _previousPaths = previousPaths};
        ...
        public static Path Root => Create(SystemPath.GetPathRoot(Current.ToString()));
        ...
        /// </summary>
        public Path Previous() => _previousPaths;

        public IEnumerator<Path> GetEnumerator() => _paths.Select(path => Create(path, this)).GetEnumerator();
        ...
        public Path Combine(params string[] pathTokens) {
            if (pathTokens.Length == 0) return (Path)this;
        ...
        /// <returns>The destination path.</returns>
        public Path Copy(string destination) => Copy(p => Create(destination, this), Overwrite.Never, false);
        ...
        /// <returns>The set of relative paths.</returns>
        public Path MakeRelativeTo(string parent) => MakeRelativeTo(Create(parent, this));
        ...
        /// <param name="destination">The destination path.</param>
        /// <returns>The destination path.</returns>
        public Path Move(string destination) => Move(p => Create(destination, this), Overwrite.Never);
        ...
        public Path Add(params string[] paths) => Create(paths.Union(_paths), this);
